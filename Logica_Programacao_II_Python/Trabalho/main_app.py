...
def app_imovel(entrada: dict) -> dict:
    ...
    meses = range(1,TEMPO_ANALISE*12+1)
    tabular = []
    for m, mc, ma, r, al, opcao in zip(meses, montante_compra, montante_aluguel, rendimentos, alugueis, opcao_mes_a_mes):
        tabular.append([m,'{:.2f}'.format(mc),'{:.2f}'.format(ma),'{:.2f}'.format(r),'{:.2f}'.format(al),'{:.2f}'.format(r-al),opcao])
    print(tabulate(tabular,headers=['Mes','Mont_Compra', 'Mont._Aluguel','Rendimento','Aluguel','Rend - Aluguel', 'Opcao']))    
